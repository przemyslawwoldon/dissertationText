public List<Double> calculateModeOfStdDev(List<Double> stdDevList) {
	...
	Map<Double, Integer> modeSpecificScope = calculateRepresentationOfValue.entrySet()
			.stream()
			.filter(map -> map.getKey() < maxStdDev)
			.collect(Collectors.toMap(p -> p.getKey(), p -> p.getValue()));

	List<Double> keyModeStdDev = new ArrayList<Double>();
	if (modeSpecificScope.size() > 0 && modeSpecificScope != null) {
		long modeStdDev = modeSpecificScope.values().stream()
			.max(Comparator.naturalOrder()).get();
		keyModeStdDev = modeSpecificScope.entrySet().stream()
			.filter(e -> e.getValue() == modeStdDev)
